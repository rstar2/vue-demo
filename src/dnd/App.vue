<template>
  <div id="app">
    <h1 class="centered">Drag And Drop</h1>
    <div>
      <h3>Some Tasks</h3>
      <ul>
    ...
    <div>
      ...
      <div class="image" v-for="(image, index) in images" v-bind:key="index"
          v-dnd v-bind:id="index" v-on:drag="handleDrag" v-on:drop="handleImageDrop">
        <img v-bind:src="image.src" v-bind:id="index" v-bind:title="image.name" align="left">
       </div>
    </div>
  </div>
</template>
